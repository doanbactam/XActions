 ...
 *
 * Uses OpenRouter to generate tweets, threads, rewrites, calendars, and replies.
 * Voice analysis has been removed; generation is now driven by topic + optional style.
 *
...
const STYLE_INSTRUCTIONS = {
  'hot-take': 'Write bold, opinionated hot takes that spark discussion. Be punchy and memorable.',
  'educational': 'Write clear, educational tweets that teach a concept or share a useful insight.',
  'personal': 'Write personal, authentic tweets that feel human and relatable.',
  'promotional': 'Write concise promotional tweets that highlight value without being too salesy.',
};

function buildSystemPrompt(style = '') {
  const styleInstruction = STYLE_INSTRUCTIONS[style] || 'Write engaging, authentic tweets that fit the topic naturally.';
  return `You are an expert Twitter/X content creator. ${styleInstruction}
- Every tweet must be under 280 characters.
- Avoid generic fluff.
- Return results as the requested JSON format.`;
}
 ...
 * @param {string} [options.style] - Optional style override
 * @param {string} [options.model] - OpenRouter model override
 * @param {string} [options.apiKey] - OpenRouter API key override
 * @returns {Promise<{ thread: Array<{ position: number, text: string, purpose: string }>, model: string }>}
 */
export async function generateThread(options = {}) {
  const { topic, length = 5, hooks = true, style, model, apiKey } = options;
 ...
 */
export async function rewriteTweet(originalText, options = {}) {
  const { goal = 'more_engaging', count = 3, style, model, apiKey } = options;
 ...
 */
export async function generateWeek(options = {}) {
  const { topics, postsPerDay = 2, days = 7, style, model, apiKey } = options;
 ...
 */
export async function generateReply(originalTweet, options = {}) {
  const { tone, count = 3, style, model, apiKey } = options;
